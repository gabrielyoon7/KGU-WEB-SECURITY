...
import kr.ac.kyonggi.cs.common.controller.LoginManager;
...
public class LoginAction implements Action{
   ...
   public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
      ...
      if(it != null) { //id 존재시 특정 신분의 유저에게는 managerLevel을 True로 줌. (양쪽 사이트에서 로그인이 가능하게.)
         if (it.type.equals("관리자") || it.type.equals("홈페이지관리자") || it.type.equals("교수1") || it.type.equals("조교") || it.type.equals("졸업논문관리자") || it.type.equals("교수2") || it.type.equals("사물함관리자") )
            managerLevel = true;
         else { //일반 학생들
            managerLevel = false;
         }
      ...
      if(it!=null) { //id 존재시
         if (it.password.equals(password)) { //로그인 성공시 (비밀번호 일치 시)
            ...
            UserBean ai_user = dao.getAIUser(it.per_id); //user_ai table에서 해당 학번 가진 학생 불러오기. 없다면 null

            //로컬에서는 http:// local~ 로컬 테스트를 위해 equals "lo" 추가
            if (url_Login.substring(7, 9).equals("ai")) { //url이 ai.kyonggi.ac.kr인 경우
//               if (url_Login.substring(7, 9).equals("ai") || url_Login.substring(7, 9).equals("lo")) { //url이 ai.kyonggi.ac.kr인 경우
               if (managerLevel || ai_user!=null) { //ai.kyonggi.ac.kr에서 ai권한을 가진 사람이 로그인 했을 때
               ...
               else {                  //ai.kyonggi.ac.kr에서 권한이 없는 사람이 로그인 했을 때
                  session.setAttribute("Access", 1); //회원 정보는 일치하나, 권한이 없는 사이트로 로그인 시 보낼 신호 ( 0 = 정상 ).
                  return "RequestDispatcher:jsp/main/login.jsp";
               }
            }
            else if (url_Login.substring(7, 9).equals("cs") || url_Login.substring(7, 9).equals("lo")) {//url이 cs.kyonggi.ac.kr인 경우
