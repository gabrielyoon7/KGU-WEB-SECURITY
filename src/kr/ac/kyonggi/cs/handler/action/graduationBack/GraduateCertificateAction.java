...
public class GraduateCertificateAction implements Action {
    @Override
    public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception{
        ...
        if(type2.type_name.equals("졸업논문관리자") || type2.type_name.contains("교수")){
            ...
            request.setAttribute("stage_data", gson.toJson(GrdBackDAO.getInstance().getCertificate(Integer.parseInt(perid)).get(0)));
            request.setAttribute("back_student", gson.toJson(GrdBackDAO.getInstance().getStudent(Integer.parseInt(perid)).get(0)));
            request.setAttribute("sc",GrdBackDAO.getInstance().getCertificate(Integer.parseInt(perid)).get(0).getAcqDate().getTime());
            request.setAttribute("download",gson.toJson(GrdBackDAO.getInstance().getCertificate(Integer.parseInt(perid)).get(0).getCerFileName()));
