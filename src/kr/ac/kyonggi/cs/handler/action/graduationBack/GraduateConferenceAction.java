...
public class GraduateConferenceAction implements Action {
    @Override
    public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception{
        ...
        */
        if(type2.type_name.equals("졸업논문관리자") || type2.type_name.contains("교수")){
            ...
            request.setAttribute("tabmenulist", gson.toJson(HomeDAO.getInstance().getGrdTabMenu(num,4)));
            request.setAttribute("stage_data", gson.toJson(GrdBackDAO.getInstance().getConference(Integer.parseInt(perid)).get(0)));
            request.setAttribute("back_student", gson.toJson(GrdBackDAO.getInstance().getStudent(Integer.parseInt(perid)).get(0)));
            request.setAttribute("sc",GrdBackDAO.getInstance().getConference(Integer.parseInt(perid)).get(0).getOpenDate().getTime());
