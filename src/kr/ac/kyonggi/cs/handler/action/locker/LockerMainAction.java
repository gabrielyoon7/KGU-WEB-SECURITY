...
import kr.ac.kyonggi.cs.handler.dto.locker.LockerAppliedUserDTO;
...
public class LockerMainAction implements Action {
    @Override
    public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        LockerAppliedUserDTO appliedStudent = LockerDAO.getInstance().getAppliedStudent(per_id);
        ...
        //신청접수 이전, 신청접수
        request.setAttribute("reqstudent", appliedStudent); //신청자 정보 불러오기
        ...
        request.setAttribute("info",gson.toJson(LockerDAO.getInstance().getinfo())); //학생회 정보 불러오기

        request.setAttribute("request", LockerDAO.getInstance().getOneSchedule(1));//신청접수
        request.setAttribute("using", LockerDAO.getInstance().getOneSchedule(2));//사물함사용
        request.setAttribute("returning", LockerDAO.getInstance().getOneSchedule(3));//반납기간
        //왜그런지 모르겠는데 JSP에서 "return"이라는 String을 getAttribute하면 error 발생함. 금지어인듯??
        return "RequestDispatcher:jsp/locker/locker_main.jsp";
    }
}
