...
public class GraduationContestAction implements Action {
	@Override
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// modify == 0(신청자 보기)
		if (modify.equals("0")) {
			GraduationContestDataDTO contest_data = GraduationTestDAO.getInstance().getConteData(perid);
			request.setAttribute("stage_data", gson.toJson(contest_data));
			request.setAttribute("download", GraduationTestDAO.getInstance().getEtcFileName(4, perid));
			request.setAttribute("download2", GraduationTestDAO.getInstance().getEtcFileName(5, perid));
			
			return "RequestDispatcher:jsp/graduation/form_etc2_test.jsp";
		}
		...
		// 자격증 데이터 테이블 default로 초기화 dao
			...
			GraduationTestDAO.getInstance().delete_contest(perid);
