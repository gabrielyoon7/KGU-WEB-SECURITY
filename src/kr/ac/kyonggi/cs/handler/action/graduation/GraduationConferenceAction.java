...
public class GraduationConferenceAction implements Action {
	@Override
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// modify == 0(신청자 보기)
		if (modify.equals("0")) {
			GraduationConferenceDataDTO conference_data = GraduationTestDAO.getInstance().getConfeData(perid);

			request.setAttribute("stage_data", gson.toJson(conference_data));
			// 논문 파일
			request.setAttribute("download", GraduationTestDAO.getInstance().getEtcFileName(2, perid));
			// 증명 파일
			request.setAttribute("download2", GraduationTestDAO.getInstance().getEtcFileName(3, perid));

			return "RequestDispatcher:jsp/graduation/form_etc3_test.jsp";
		}
		...
		// 자격증 데이터 테이블 default로 초기화 dao
			...
			GraduationTestDAO.getInstance().delete_conference(perid);
