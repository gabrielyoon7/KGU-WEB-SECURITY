...
import kr.ac.kyonggi.cs.v2.handler.excel.ExcelWriter;
...
public class ExcelAction implements Action{
   ...
   public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
      ...
      GraduationCheckingSystemDAO lecture = GraduationCheckingSystemDAO.getInstance();
      Gson gson=new Gson();
      if(writeorread.equals("write")) {
         ExcelWriter ex = new ExcelWriter();
         
//         UserTypeBean type1 = gson.fromJson((String)request.getSession().getAttribute("type"), UserTypeBean.class);
//         if(type1.board_level>0)
//            return null;
          String savePath = request.getServletContext().getRealPath("/uploadFile"); //저장경로
          String file_name = ex.xlsWriter(lecture.getLecture(),savePath);
...
//            }else
               result =gson.toJson(ex.xlsxLectureReader(address));
         }else {
            ExcelReader ex = new ExcelReader();
//            if(data!=null) {
//               result=gson.toJson(ex.xlsUserReadermodify(data,address));
//            }else
               result=gson.toJson(ex.xlsLectureReader(address));
         }
      }
