...
<%
   StringBuffer url2_grd_form_etc3 = request.getRequestURL();
   String logo_img_grd_form_etc3;

   //로컬에서는 http:// local~ 로컬 테스트를 위해 equals "lo" 추가
   //ai이면 ai로고 cs이면 cs로고
   if(url2_grd_form_etc3.substring(7,9).equals("ai") || url2_grd_form_etc3.substring(7,9).equals("lo")){
      logo_img_grd_form_etc3 = "img/graduation_ai.png";
   }
   else{
      logo_img_grd_form_etc3 = "img/graduation.png";
   }
   //System.out.println((logo_img_grd_form_etc3));
%>
<%
   ...
   String download=(String) request.getAttribute("download");	//학술대회1(논문파일)
   String download2=(String) request.getAttribute("download2");	//학술대회2(증명 파일)
...
<main>
   <div id="content">
      <div id="title">
         <img src=<%=logo_img_grd_form_etc3%> />
         <div>졸업논문</div>
      </div>
      <div id="container">
         <div id="tab">
                  ...
                  <ul>
                     ...
                     <li>
                        <div id="uploadfile1" style="display:flex">
                           ...
                           <div style="margin-right:auto"><button class="btn btn-default" onclick="uploadfile(8)" style="padding: 3px 10px"><img src="img/uploadBtn.png"></button></div>
                        </div>
                     </li>
                     <li>
                        <div id="uploadfile2" style="display:flex">
                           ...
                           <div style="margin-right:auto"><button class="btn btn-default" onclick="uploadfile(9)" style="padding: 3px 10px"><img src="img/uploadBtn.png"></button></div>
                        </div>
                     </li>
                  </ul>
               </div>
...
<script>
   ...
   function usertable(grduser, graduationuser) {
      ...
      a += '</tr>';
      a += '<tr style="border-bottom : 1px solid #ddd">';
      a += '<td style="background-color : #ECEFF1; font-weight: 600;">이름</td><td>' + grduser.name + '<td style="background-color : #ECEFF1; font-weight: 600;">소속학과</td><td>' + graduationuser.major
   ...
   if(modify == 0 || modify == 1 || modify == 4 || modify == 5){
      ...
      var a='';
      a+='<ul><li><div style="display : flex"><div class="profile">학술대회명</div><div class="form-group"><input type="text" class="form-control" id="contest_name" placeholder="제목을 입력하세요" value="' + stage_data.conference_name + '" style="width : 620px"></div></li>';
      ...
      a+='<li><div style="display: flex"><div class="profile">논문제목</div><div class="form-group"><input type="text" class="form-control" id="thesis" placeholder="제목을 입력하세요" style="width : 620px" value="' + stage_data.thesis_title + '"></div></li>';
      a+='<li><div style="display:flex"><div class="profile">개최기관</div><div class="form-group"><input type="text" class="form-control" id="openplace" style="width : 263px;" placeholder="개최기관을 입력하세요" value="' + stage_data.organization + '"></div><div class="profile" style="margin-left : 10px">학회개시일</div><input type="date" class="form-control" style="width:263px; margin-left:5px;" id="Inputdate" name = "new_date" value="' + formatDateToNum(stage_data.open_date) + '"></div></li>';
      a+='<li><div id="uploadfile1" style="display:flex"><div class="profile">논문파일</div>';
      a+='<div style="margin-left: 5px">'+download+'</div><div style="margin-right:5px; margin-left : 10px"><a href="download.do?id='+graduationuser.per_id+'&stage=8"><button class="btn btn-default" style="padding : 3px 10px;"><img src="img/downloadBtn.png"></button></a><button class="btn btn-default" onclick="modifyfilebutton(8)" style="padding: 3px 10px; margin-left : 5px"><img src="img/removeBtn.png"></button></div></div></li>';
      a+='<li><div id="uploadfile2" style="display:flex"><div class="profile">기타증빙</div>';
      a+='<div style="margin-left: 5px">'+download2+'</div><div style="margin-right:5px; margin-left : 10px"><a href="download.do?id='+graduationuser.per_id+'&stage=9"><button class="btn btn-default" style="padding : 3px 10px;"><img src="img/downloadBtn.png"></button></a><button class="btn btn-default" onclick="modifyfilebutton(9)" style="padding: 3px 10px; margin-left : 5px"><img src="img/removeBtn.png"></button></div></div></li></ul>';
      modifymain.html(a);
      $('input:checkbox').each(function(){
         if(stage_data.requirement.includes($(this).val()))
   ...
   if(modify == 0 || modify == 4){
      ...
      b+='<a onclick = "insert_etc3()"><button type="button" class="btn btn-default">수정</button></a><a onclick="printview()" class="btn btn-default">인쇄</a></div>';
   ...
   function modifyfilebutton(sta){
      var a='';
      if(sta==8){
         a+='<div class="profile">논문파일</div><div style="margin-left:5px; margin-top : 7px"><input type="file" name="uploadFile" id="uploadFile" accept=".hwp, .doc, .docx, .pdf"></div><div><button class="btn btn-default" onclick="uploadfile(' + sta + ')" style="padding: 3px 10px"><img src="img/uploadBtn.png"></button></div>';
         a+='<div style="margin-left : 5px"><button class="btn btn-default" onclick="cancelfile(' + sta + ')" style="padding : 6px 10px">취소</button></div>';
         $('#uploadfile1').html(a);
      }else if(sta==9){
         a+='<div class="profile">기타증빙</div><div style="margin-left:5px; margin-top : 7px"><input type="file" name="uploadFile2" id="uploadFile2" accept=".hwp, .doc, .docx, .pdf"></div><div><button class="btn btn-default" onclick="uploadfile(' + sta + ')" style="padding: 3px 10px"><img src="img/uploadBtn.png"></button></div>';
         a+='<div style="margin-left : 5px"><button class="btn btn-default" onclick="cancelfile(' + sta + ')" style="padding : 6px 10px">취소</button></div>';
   ...
   function insert_etc3(){//학술대회 제출(required 필요함)
      ...
      if(signature==user.name){
         ...
         if(modify=="1"){
            modi="modify";
            if(obj.upload1==null)
               obj.upload1=stage_data.thesis_filename;
            if(obj.upload2==null){
               obj.upload2=stage_data.proof_filename;
            }
         }
   ...
   function printview(){
      ...
      $("#dele1").html('<a href="download.do?id='+graduationuser.per_id+'&stage=8"><button class="btn btn-default" style="padding : 3px 10px; margin-left : 10px"><img src="img/downloadBtn.png"></button></a>');
      $("#dele2").html('<a href="download.do?id='+graduationuser.per_id+'&stage=9"><button class="btn btn-default" style="padding : 3px 10px; margin-left : 10px"><img src="img/downloadBtn.png"></button></a>');
   }
