...
<%
    StringBuffer url2_grd_form_mid = request.getRequestURL();
    String logo_img_grd_form_mid;

    //로컬에서는 http:// local~ 로컬 테스트를 위해 equals "lo" 추가
    //ai이면 ai로고 cs이면 cs로고
    if(url2_grd_form_mid.substring(7,9).equals("ai") || url2_grd_form_mid.substring(7,9).equals("lo")){
        logo_img_grd_form_mid = "img/graduation_ai.png";
    }
    else{
        logo_img_grd_form_mid = "img/graduation.png";
    }
    //System.out.println((logo_img_grd_form_mid));
%>
<%
...
<body>
   ...
   var grduser=<%=grduser%>;
      ...
      var download=<%=download%>
        if(modify==1){//name(answer_1), checked_value(answer_2), keyword(answer_3), content(answer_4)
          ...
          var a='';  
          a+='<ul><li><div style="display: flex"><div class="profile">제목</div><div class="form-group"><input type="text" class="form-control" id="mid_name" placeholder="제목을 입력하세요" style="width : 620px" value="' + stage_data.interim_title + '"></div></div></li>';
         ...
         a+='<li><div style="display:flex"><div class="profile" style="padding-top : 50px">진행내용</div><div class="form-group" style="display : inline-block; width : 620px;"><textarea class="form-control" id="progress_content" rows=6 placeholder="진행내용을 입력하세요">' + stage_data.interim_content + '</textarea></div></div></li>';   
         a+='<li><div style="display:flex"><div class="profile" style="padding-top : 50px">향후계획</div><div class="form-group" style="display : inline-block; width : 620px;"><textarea class="form-control" id="plan_content" rows=6 placeholder="향후계획을 입력하세요">' + stage_data.interim_plan + '</textarea></div></div></li></ul>';
         modifymain.html(a);  
         }else if(modify==2){
              var modifymain=$('#modify_main');
              var sign=$('#sign');
             ...
             sign.html(b);
             a+='<ul><li><div class="profile">제목 </div><div class="inform">'+stage_data.interim_title+'</div></li>';
            a+='<li><div id="thesis_what" style="display:flex"><div class="profile">구분 </div></div></li>';
            a+='<li><div id="uploadfile1" style="display:flex"><div class="profile">파일제출</div>';   
            a+='<div style="margin-left:5px">'+download[0]+'</div><div style="margin-left:5px"><a href="download.do?id='+graduationuser.per_id+'&stage=3"><button class="btn btn-default" style="padding : 3px 10px; margin-left : 10px"><img src="img/downloadBtn.png"></button></a></div></div></li>';   
            a+='<li><div style="display:flex"><div class="profile" style="padding-top : 50px">진행내용</div><div class="form-group" style="display : inline-block; width : 620px;"><textarea class="form-control" id="progress_content" rows=6 placeholder="진행내용을 입력하세요" readonly>' + stage_data.interim_content + '</textarea></div></div></li>';   
            a+='<li><div style="display:flex"><div class="profile" style="padding-top : 50px">향후계획</div><div class="form-group" style="display : inline-block; width : 620px;"><textarea class="form-control" id="plan_content" rows=6 placeholder="향후계획을 입력하세요" readonly>' + stage_data.interim_plan + '</textarea></div></div></li></ul>';
             modifymain.html(a);  
         }
   ...
   function uploadfile(){
            ...
            formData.append("stage",3);   //추가한거
            $.ajax({
               url : 'upload.do?writer='+graduationuser.per_id+'&stage=3',
               type : "post",
               async:false,
               data : formData,
                 processData : false,
