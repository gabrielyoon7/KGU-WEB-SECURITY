...
<%
	StringBuffer url2_grd_ulist = request.getRequestURL();
	String logo_img_grd_ulist;

	//로컬에서는 http:// local~ 로컬 테스트를 위해 equals "lo" 추가
	//ai이면 ai로고 cs이면 cs로고
	if(url2_grd_ulist.substring(7,9).equals("ai") || url2_grd_ulist.substring(7,9).equals("lo")){
		logo_img_grd_ulist = "img/graduation_ai.png";
	}
	else{
		logo_img_grd_ulist = "img/graduation.png";
	}
//	System.out.println((logo_img_grd_ulist));
%>
<%
...
<body>
					...
					<table id="insert_excel" data-toggle="table" data-height="299">
						<thead>
							<tr>
								...
								<th data-field="name" data-sortable="true">이름</th>
								<th data-field="graduation_date" data-sortable="true">졸업년도</th>
								<th data-field="major" data-sortable="true">소속학과</th>
								<th data-field="prof_name" data-sortable="true">지도교수</th>
								<th data-field="capstone_str" data-sortable="true">캡스톤여부</th>
							</tr>
						</thead>
					</table>
				</div>
	...
	<!-- Modal3 -->
	<div class="modal fade" id="myModal_req" role="dialog">
		<div class="modal-dialog">
			<!-- Modal content-->
			<div class="modal-content">
				<div class="modal-header">
				...
				<div id="howmany_req"></div>
				<div class="modal-body" id="myModalbody_req">
					<table id="insert_excel_req" data-toggle="table" data-height="299">
						<thead>
							<tr>
								<th data-field="state" data-checkbox="true"></th>
								<th data-field="number" data-sortable="true">번호</th>
				...
				<div class="modal-footer" id="footer_req"></div>
			</div>
		</div>
	</div>
	<!-- Modal4 -->
	<div class="modal fade" id="myModal_req2" role="dialog">
		<div class="modal-dialog">
			<!-- Modal content-->
			<div class="modal-content">
				<div class="modal-header">
				...
				<div id="howmany_req2"></div>
				<div class="modal-body" id="myModalbody_req2"></div>
				<div class="modal-footer" id="footer_req2"></div>
			</div>
		</div>
	</div>
	<script>
   makeboard(tab_2);
               ...
               if(type.type_name == '졸업논문관리자' && <%=ordernum%> == 6){
            	   var a='';
            	   a+='<div class="col-xs-8"><input type="file" name="uploadFile_req" id="uploadFile_req" accept=".xls, .xlsx"></div>';
            	   //a+='<a href="graduationReqExcelDown.do"><button type="button" class="btn btn-default col-xs-1" style="height: 34px;">다운</button>';
            	   a+='<a href="#myModal_req2" data-toggle="modal" onclick="insertgrduser_req()"class="btn btn-default col-xs-1">추가</a>';
            	   a+='<a href="#myModal_req" data-toggle="modal" onclick="grd_excel_req()"><button type="button" class="btn btn-default col-xs-1" style="height: 34px;">엑셀업</button></a>';
	               a+='<a onclick="deletegrduser_req()" class="btn btn-default" style="height: 34px; float: right">삭제</a>';
	               buttonmenu.html(a);
               }
                   ...
                   if(reqorgrd==1){
                	   for(var i=0;i<users.length;i++){
                           ...
                           rows.push({
                              index:i+1,
                               per_id:value.per_id,
                               name:'<a href="graduation_manage.do?num=94&perid='+users[i].per_id+'">'+users[i].name+'</a>',
                               prof_name:value.prof_name,
                               graduation_date:value.graduation_date,
                               grd_state:value.current_state,
                               grd_state_level:value.current_str,
                               capstone : cap
                           });
                        }
                   }
                	   ...
                	   for(var i=0;i<users.length;i++){
                           ...
                           rows.push({
                              index:i+1,
                               per_id:users[i].per_id,
                               name:'<a href="graduation_manage.do?num=94&perid='+users[i].per_id+'">'+users[i].name+'</a>',
                               prof_name:users[i].prof_name,
                               graduation_date:users[i].graduation_date,
                               grd_state:users[i].current_state_string,
                               grd_state_level:users[i].current_str,
                               capstone : cap
                           });
                        }
                   }
          ...
          var addgrd_req=null;
          function grd_excel_req(){
                   var address = uploadexcel_req();
                   $.ajax({
                      url : "graduationexcel_req.do",
                      type : "post",
                      async : false,
                      data : {
                         address : address
                      ...
                      success : function(data){
                    	 addgrd_req=data;
                         var modal = $('#MyModalbody_req');
                         var howmany =$('#howmany_req');
                         var a = '<h3> 총 ' + (data.length)+ '명의 회원이 등록될 예정입니다.</h3>';
                         howmany.html(a);
                         var table=$('#insert_excel_req');
                         table.bootstrapTable('load',insertdata_req(data));
                         table.bootstrapTable('refresh');
                         var footer = $('#footer_req');
                         footer.html('<button id="deletegrd_req" class="btn btn-default" onclick="deletegrd_req('+data.length+')">선택 삭제</button><button id="addbutton_req" type="button" class="btn btn-default" style = "margin : 1px;" onclick="addgrduser_req()">일괄 추가</button>');
                      }
                   })
                }          //여기까지
                ...
                function insertdata(data){
                    ...
                    for(var i=0;i<data.length;i++){
                       ...
                       rows.push({
                           ...
                           major:value.major,
                           prof_name:value.prof_name,
                           capstone_str:value.capstone
                       });
                    }
                    return rows;
                 }  
                
          function insertdata_req(data){
              var rows = [];
              for(var i=0;i<data.length;i++){
                 var value=data[i];
                 rows.push({
         ...
         function uploadexcel_req(){
            var formData = new FormData();
            var address="";
            formData.append("excelfile",$('input[name=uploadFile_req]')[0].files[0]);
            $.ajax({
               url : "insertExcel.do",
               type : "post",
               async: false,
         ...
         function addgrduser(){
             ...
             $.ajax({
                ...
                success : function(data){
                   var text = data[data.length-1]+"명의 인원이 추가되었고 "+data[data.length-2]+"명의 인원이 중복되었습니다.\n제외된 인원들  " + (data.length - 2) + "명(유저에 없는 학번) : ";
         ...
         function insertgrduser(){
             ...
             a += '<div class="form-group"><label for="Inputdate" style="display :inline-block">졸업년도</label><select class="form-control" id="Inputdate"><option>'+(year)+'-08</option><option>'+(year+1)+'-02</option><option>'+(year+1)+'-08</option></select></div>';
             a += '<div class="form-group" id="major1"><label for="InputMajor">학과</label><input type="text" class="form-control" id="InputMajor" placeholder="학과 입력해주세요"></div>';
         ...
         function insertgrduser_req(){
        	 var proflist=<%=proflist%>;
             var list = $('#myModalbody_req2');
             var today = new Date();
             var year = today.getFullYear();
             var a = '<div class="form-group"><label for="InputID">학번</label><span id="warningID"></span><div class="input-group"><input onkeypress="changeID()" type="text" class="form-control" placeholder="학번를 입력해주세요" id="InputID"><span class="input-group-btn"><button class="btn btn-default" type="button" onclick="checkID_req();">학번확인</button></span></div></div>';
             a += '<div class="form-group" id="name1"><label for="InputName">이름</label><input type="text" class="form-control" id="InputName" placeholder="이름을 입력해주세요"></div>';
             a+='</select></div>';
             a += '<div class="form-group"><label for="Inputdate" style="display :inline-block">졸업년도</label><select class="form-control" id="Inputdate"><option>'+(year)+'-08</option><option>'+(year+1)+'-02</option><option>'+(year+1)+'-08</option></select></div>';
             a += '<div class="form-group" id="major1"><label for="InputMajor">학과</label><input type="text" class="form-control" id="InputMajor" placeholder="학과 입력해주세요"></div>';
             a += '<div class="col-xs-13 text-right"><button class="btn btn-default" onclick="insertonegrduser_req()">회원가입</button></div>';
             a += '<input type="submit" onclick="insertonegrduser_req()" style="display : none">';
              list.html(a);
          }
      ...
      function insertonegrduser(){
          if($('#warningID').text() == '*존재하는 학번 & 졸업논문 대상자에 들어있지 않습니다'){
                ...
                if(name!='' && per_id!='' && prof_name!='' && date!='' && major!=''){
      ...
      function insertonegrduser_req(){
            if($('#warningID').text() == '*존재하는 학번 & 졸업논문 대상자에 들어있지 않습니다'){
                  ...
                  var prof=$('#Inputprofessor').val();
                  if(name!='' && per_id!='' && date!='' && major!=''){
                     var update = per_id+"-/-/-"+name+"-/-/-"+date+"-/-/-"+major;
                     $.ajax({
                        url:"ajaxuser.do",
                        type:"post",
                        data:{
                           req:"insertonegrduser_req",
                           data: update
                        },
                        success:function(data){
                           alert("학번 : "+data+"  추가 완료");
