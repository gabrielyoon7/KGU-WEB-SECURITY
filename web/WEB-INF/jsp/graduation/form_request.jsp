...
<%
   ...
   String stage_data = (String) request.getAttribute("stage_data");//저장된 grd_log
   String tabmenulist = (String) request.getAttribute("tabmenulist");//관리자탭메뉴
   String grduser = (String) request.getAttribute("grduser");//유저객체
   String subDBuser = (String) request.getAttribute("graduationuser");//정식일떈정식,sub일땐sub에서 찾은 grd객체
   String modify = (String) request.getAttribute("modify");//0=제출//1=수정//2=보기
...
11월 10일 최종논문제출 및 지도교수확인서 오프라인제출</textarea>
   ...
   for(var i=0;i<schedulelist.length;i++){
      var closing=new Date(schedulelist[i].closing_date);
   ...
   function set_student_Data(){//학생정보 조건문에따라 설정
      ...
      if((user.type.includes("관리자"))||(user.type.includes("교수"))){
         if(grduser.per_id==null){
            ...
            +'<li><div class="profile">소속학과</div><div class="inform"><input type="text" name="major" placeholder="예)컴퓨터과학과" /></div></li>'
         }
         else{
            ...
            +'<li><div class="profile">소속학과</div><div class="inform"><input type="text" name="major" placeholder="예)컴퓨터과학과" value="'+grduser.major+'" readonly/></div></li>'
         }
      }else{
         if(user.per_id==null){
            a+='<li><div class="profile">학번</div><div class="inform"><input type="text" name="per_id" placeholder="학번을 입력해주세요"/></div></li>'
            ...
            +'<li><div class="profile">소속학과</div><div class="inform"><input type="text" name="major" placeholder="예)컴퓨터과학과" value="'+user.major+'" readonly/></div></li>'
         }
      }
            list.prepend(a);
   }
   ...
   function insertsemester(){//관리자/교수거나 보기이면 날짜는 div로  수정/제출이면 select로 설정
    ...
    function submit_request(){//신청서 접수하는 function
        ...
        var user = <%=user%>
        var answer = user.per_id+"-/-/-"+user.name+"-/-/-"+date+"-/-/-"+check;
        if(check!=0){
         $.ajax({
            url:"ajax.do",
               type:"post",
