   ...
   var grduser=<%=user%>;
    ...
    var download=<%=download%>;
     if(modify==1){
          ...
          var a='';  
          a+='<ul><li><div style="display:flex"><div class="profile">공모전명</div><div class="form-group"><input type="text" class="form-control" id="contest_name" placeholder="제목을 입력하세요" style="width : 620px" value="' + stage_data.answer_1 + '"></div></div></li>';
         ...
         a+='<li><div style="display:flex"><div class="profile">시상내역</div><div class="form-group"><input type="text" class="form-control" id="price" style="width : 263px;" placeholder="직접 입력하세요" value="' + stage_data.answer_3 + '"></div><div class="profile" style="margin-left : 10px">개최기관</div><div class="form-group"><input type="text" class="form-control" id="openorgan" style="width : 263px;" placeholder="직접 입력하세요" value="' + stage_data.answer_4 + '"></div></div>';
         a+='<li><div style="display:flex"><div class="profile">시상일</div><input type="date" class="form-control" style="width:263px; margin-left : 5px;" id="receivedate" name = "new_date" value ="'+stage_data.answer_5+'"><div class="profile" style="margin-left : 10px">대회개최일</div><input type="date" class="form-control" style="width:263px; margin-left : 5px;" id="opencontest" name = "new_date" value ="'+stage_data.answer_6+'"></div>';
         a+='<li><div id="uploadfile1" style="display:flex"><div class="profile">상장사본</div>';
         a+='<div style="margin-left: 5px">'+download[0]+'</div><div style="margin-right:5px; margin-left : 10px"><a href="download.do?id='+stage_data.answer_7+'"><button class="btn btn-default" style="padding : 3px 10px;"><img src="img/downloadBtn.png"></button></a><button class="btn btn-default" onclick="modifyfilebutton(1)" style="padding: 3px 10px; margin-left : 5px"><img src="img/removeBtn.png"></button></div></div></li>';
         a+='<li><div id="uploadfile2" style="display:flex">';
         a+='<div class="profile">추가자료</div>';
         a+='<div style="margin-left: 5px">'+download[1]+'</div><div style="margin-right:5px; margin-left : 10px"><a href="download.do?id='+stage_data.answer_8+'"><button class="btn btn-default" style="padding : 3px 10px;"><img src="img/downloadBtn.png"></button></a><button class="btn btn-default" onclick="modifyfilebutton(2)" style="padding: 3px 10px; margin-left : 5px"><img src="img/removeBtn.png"></button></div></div></li></ul>';
         modifymain.html(a); 
            $('input:radio[name=select_license]').each(function(){
               if($(this).val()==stage_data.answer_2)
             ...
             var b='<div class="text-right"><a href="javascript:history.go(-1)"><button type="button" style = "margin : 2px;" class="btn btn-default">취소</button></a>';
               if((user.type.includes("관리자")||user.type.includes("교수"))){
                  ...
                  if(stage_data.success_date==null)
                          b+='<a href="graduation_form.do?per_id='+graduationuser.per_id+'&num=94&stage=5&modify=1"><button type="button" class="btn btn-default">수정</button></a><a onclick="printview()" class="btn btn-default">인쇄</a></div>';
             sign.html(b);
             etc_button.html(a);
             a+='<ul><li><div style="width:709px;"><div class="profile">공모전명</div><div class="inform">'+stage_data.answer_1+'</div></div></li>';
            a+='<li><div style="display:flex"><div class="profile">팀유형</div><div>';
            a+='<div class="inform" style="margin-right:auto;font-size:12px;">'+stage_data.answer_2+'</div></div></li>';
            a+='<li><div style="display:flex"><div class="profile">시상내역</div><div class="inform" style="width: 263px;">'+stage_data.answer_3+'</div>';
            a+='<div class="profile" style="margin-left : 10px">개최기관</div><div class="inform"  style="width : 300px;">'+stage_data.answer_4+'</div></div></li>';
            a+='<li><div style="display:flex"><div class="profile">시상일</div><input type="date" class="form-control" style="width:263px; margin-left :5px" id="receivedate" name = "new_date" value ="'+stage_data.answer_5+'"  readonly>';
            a+='<div class="profile" style="margin-left : 10px">대회개최일</div><input type="date" class="form-control" style="width:263px; margin-left :5px" id="opencontest" name = "new_date" value ="'+stage_data.answer_6+'" placeholder="Date of Birth" readonly></div></li>';
            a+='<li><div id="uploadfile1" style="display:flex"><div class="profile">상장사본</div>';
            a+='<div style="margin-left:5px">'+download[0]+'</div><div id="dele1" style="margin-left:5px"><a href="download.do?id='+stage_data.answer_7+'"><button class="btn btn-default" style="padding : 3px 10px; margin-left : 10px"><img src="img/downloadBtn.png"></button></a></div></div></li>';
            a+='<li><div id="uploadfile2" style="display:flex">';
            a+='<div class="profile">추가자료</div>';
            a+='<div style="margin-left:5px">'+download[1]+'</div><div id="dele2" style="margin-left:5px"><a href="download.do?id='+stage_data.answer_8+'"><button class="btn btn-default" style="padding : 3px 10px; margin-left : 10px"><img src="img/downloadBtn.png"></button></a></div></div></li></ul>';
            modifymain.html(a);  
    ...
    function modifyfilebutton(id){
       var a='';
       if(id==1){ 
             a+='<div class="profile">상장사본</div><div style="margin-left:5px; margin-top : 7px"><input type="file" name="uploadFile" id="uploadFile" accept=".hwp, .doc, .docx, .pdf"></div><div><button class="btn btn-default" onclick="uploadfile(' + id + ')" style="padding: 3px 10px"><img src="img/uploadBtn.png"></button></div>';
             a+='<div style="margin-left : 5px"><button class="btn btn-default" onclick="cancelfile(' + id + ')" style="padding : 6px 10px">취소</button></div>';
               $('#uploadfile1').html(a);
       }else if(id==2){
           a+='<div class="profile">추가자료</div><div style="margin-left:5px; margin-top : 7px"><input type="file" name="uploadFile2" id="uploadFile2" accept=".hwp, .doc, .docx, .pdf"></div><div><button class="btn btn-default" onclick="uploadfile(' + id + ')" style="padding: 3px 10px"><img src="img/uploadBtn.png"></button></div>';
             a+='<div style="margin-left : 5px"><button class="btn btn-default" onclick="cancelfile(' + id + ')" style="padding : 6px 10px">취소</button></div>';
            $('#uploadfile2').html(a);
       }
       ...
       if(signature==user.name){
         ...
         for (var i = 0; i < check.length; i++) {
            ...
            if(modify=="1"){
              modi="modify";
              if(obj.file1==null){
                 obj.file1=stage_data.answer_7;
              }
              if(obj.file2==null){
                 obj.file2=stage_data.answer_8;
              }
           }
        ...
        function printview(){
            ...
            $("#dele1").html('<a href="download.do?id='+stage_data.answer_7+'"><button class="btn btn-default" style="padding : 3px 10px; margin-left : 10px"><img src="img/downloadBtn.png"></button></a>');
            $("#dele2").html('<a href="download.do?id='+stage_data.answer_8+'"><button class="btn btn-default" style="padding : 3px 10px; margin-left : 10px"><img src="img/downloadBtn.png"></button></a>');
         }
