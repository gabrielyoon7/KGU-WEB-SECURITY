...
<%
   StringBuffer url2_professor = request.getRequestURL();
   String logo_img_professor;

   //로컬에서는 http:// local~ 로컬 테스트를 위해 equals "lo" 추가
   //ai이면 ai로고 cs이면 cs로고
   if(url2_professor.substring(7,9).equals("ai") || url2_professor.substring(7,9).equals("lo")){
      logo_img_professor = "img/member_ai.png";
   }
   else{
      logo_img_professor = "img/member.png";
   }
   //System.out.println((logo_img_professor));
%>
<%
...
<meta charset="utf-8">
<title>교수소개 : 경기대학교 AI컴퓨터공학부</title>
...
//            pane.prepend('<img src="'+headtitle[i].tab_img+'" alt="">');
   ...
   <%=professorList%>
      ...
      for (var i = 0; i < prolist.length; ++i) {
         propanel
               .append('<div id="professor_profile' + (prolist[i].id) + '"><div id="image'+ (prolist[i].id) +'" style="display : inline"><img src="img/professor/'+prolist[i].prof_img+'" alt="" class="profile"></div>'
