...
//	pane.prepend('<img src="'+headtitle[6].tab_img+'" alt="">');
	...
	function modifyBoard(){
		window.location.href = 'gallery_board_modify.do?id=' + <%=id%> + '&num=' + <%=num%>;
	}
	...
	for(var i = 0 ; i < images.length ; ++i){
     ...
     if(nlist[0].title==undefined)
        ...
        next.append('<a href="article_reader.do?id='+nlist[0].id+'&num='+num+'">'+nlist[0].title+'</a>');
     if(nlist[1].title==undefined)
        previous.append('이전글이 없습니다.');
     else
        previous.append('<a href="article_reader.do?id='+nlist[1].id+'&num='+num+'">'+nlist[1].title+'</a>');
