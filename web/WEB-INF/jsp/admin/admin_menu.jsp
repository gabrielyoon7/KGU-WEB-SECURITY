...
<%
    StringBuffer url2_admin_menu = request.getRequestURL();
    String logo_img_admin_menu;

    //로컬에서는 http:// local~ 로컬 테스트를 위해 equals "lo" 추가
    //ai이면 ai로고 cs이면 cs로고
    if(url2_admin_menu.substring(7,9).equals("ai") || url2_admin_menu.substring(7,9).equals("lo")){
        logo_img_admin_menu = "img/notice_ai.png";
    }
    else{
        logo_img_admin_menu = "img/notice.png";
    }
    //System.out.println((logo_img_admin_menu));
%>
<% 
    ...
    for(var i=0;i<menulist.length;i++){
       ...
       if(value.tab_id<8){
          ...
          if(value.show_in_menus==1){
             if(value.path=="information.do"){
                 typeName = "정보 페이지";
             }
             else if(value.path=="notice_article_list.do"){
                typeName = "공지사항 게시판";
             }
             else{
                typeName = "ETC";
      ...
      $.ajax({
         ...
         if(it.path=="notice_article_list.do"){
