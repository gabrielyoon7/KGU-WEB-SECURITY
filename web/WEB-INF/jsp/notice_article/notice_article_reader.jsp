...
<%
    StringBuffer url2_na_reader = request.getRequestURL();
    String logo_img_na_reader;

    //로컬에서는 http:// local~ 로컬 테스트를 위해 equals "lo" 추가
    //ai이면 ai로고 cs이면 cs로고
    if (url2_na_reader.substring(7, 9).equals("ai") || url2_na_reader.substring(7, 9).equals("lo")) {
        logo_img_na_reader = "img/notice_ai.png";
    } else {
        logo_img_na_reader = "img/notice.png";
    }
    //System.out.println((logo_img_na_reader));
%>
<%
...
function makelistbutton(num) {
    return '<div><a href="notice_article_list.do?num=' + num + '" class="btn btn-default">목록</a></div>';
}

function makemodifybutton(str, num) {
    return '<div><a href="notice_article_modifier.do?num=' + num + '&id=' + str.id + '" id="modifier" class="btn btn-default">수정</a></div>';
}
...
for (var i = 0; i < file.length; i++) {
    ...
    a += '<a href="notice_download.do?id=' + it.id + '">' + it.filename + '</a>&nbsp&nbsp';
else
...
if (nlist[0].title == undefined)
    ...
    next.append('<a href="notice_article_reader.do?id=' + nlist[0].id + '&num=' + num + '">' + nlist[0].title + '</a>');
if (nlist[1].title == undefined)
    previous.append('이전글이 없습니다.');
else
    previous.append('<a href="notice_article_reader.do?id=' + nlist[1].id + '&num=' + num + '">' + nlist[1].title + '</a>');
</script>
</body>
</html>