...
function makelistbutton(num){
    return '<div><a href="webzine_list.kgu?num=' + num + '" class="btn btn-default">목록</a></div>';

}
function makemodifybutton(str,num){
    return '<div><a href="webzine_modifier.kgu?num='+num+'&id='+str.id+'" id="modifier" class="btn btn-default">수정</a></div>';
}
...
for(var i = 0 ; i < file.length ; i++){
    ...
    a += '<a href="webzine_download.kgu?id=' + it.id + '">' + it.filename + '</a>&nbsp&nbsp';
else
...
function likeIt(){
    ...
    $.ajax({
        ...
        success : function(data){
            if(data == 'success'){
                alert('추천 성공');
                window.location.href = 'webzine_reader.kgu?id=' + <%=id%> + '&num=' + <%=num%>;
...
if(nlist[0].title==undefined)
    ...
    next.append('<a href="webzine_reader.kgu?id='+nlist[0].id+'&num='+num+'">'+nlist[0].title+'</a>');
if(nlist[1].title==undefined)
    previous.append('이전글이 없습니다.');
else
    previous.append('<a href="webzine_reader.kgu?id='+nlist[1].id+'&num='+num+'">'+nlist[1].title+'</a>');
</script>
</body>
</html>
