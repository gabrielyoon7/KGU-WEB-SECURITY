...
<script>
    ...
    if(type.board_level <= 3)
        write.append( '<a href="req_article_writer.kgu" class="btn btn-default">글쓰기</a>');
        ...
        for(var i=0;i<boards.length;i++){
            ...
            if(user != null){
                if(value.student_id==user.id){
                    lvlText = '<a href="req_article_reader.kgu?id='+value.id+'&num=' + <%=num%> + '">'+value.title+'</a>';
                    can = '작성자';
                }
                else if(value.level.indexOf(type.for_header) >= 0 || type.for_header == '관리자'){
                    lvlText = '<a href="req_article_reader.kgu?id='+value.id+'">'+value.title+'</a>';
                    can = '○';
                }
                else{
    ...
    function goReqBoard(id){
        window.location.href = 'req_article_reader.kgu?id='+id;
    }
